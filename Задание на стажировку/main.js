...
let jsonDb = [
  ...
  let table = document.querySelector('.table');// Нахождение таблицы, в которую буду вписывать значения
  let tbody = document.createElement('tbody')
  for(let i = 0; i < jsonDb.length; i = i + 1){
      let tr = document.createElement('tr'); // создание строки таблицы
      
      for(let j = 0; j < 5; j++){
        let td = document.createElement('td');//содание столбцов строки
        if(j==0){
          td.innerHTML = i + 1; // Заполнение столбцов порядковым номер
        }
        else if(j==1){// Заполнение столбцов данными из jsonDb.name.firstName
            td.innerHTML = jsonDb[i].name.firstName;
            td.style.textAlign = 'center';
        }
        else if(j==2){// Заполнение столбцов данными из jsonDb.name.lastBane
            td.innerHTML = jsonDb[i].name.lastName;
            td.style.textAlign = 'center';
        }
        else if(j==3){// Заполнение столбцов данными из jsonDb.about
            if(jsonDb[i].about.length > 180){//Обрезка данных из about до двух строк и добавление в конце ...
              td.innerHTML = jsonDb[i].about.substring(0,180) + '...';
              td.onclick = function(){// функция раскрытия данных из about
                td.innerHTML = jsonDb[i].about;
                td.title = 'кликните дважды для скрытия';
            }
            td.ondblclick = function(){// функция скрытия данных из about
                td.innerHTML = jsonDb[i].about.substring(0,209) + '...';
            }
        ...
        else if(j==4){// Заполнение столбцов данными из jsonDb.eyeColor
            td.value = jsonDb[i].eyeColor;// добавление данных из eyeColor в value для возможности сортирвки
            td.style.backgroundColor = jsonDb[i].eyeColor;// Цветовое отображение
            td.innerHTML = jsonDb[i].eyeColor;// Добавление данных из eyeColor во внутрь тэга td, ддля возможности дальнейшего редактирования строки
            td.style.fontSize = 0;
        }
        tr.appendChild(td);// Добавление стобцов в строку
      }
      if(i > 9){//Условие для выполнения постраничного вывода
        tr.style.display = 'none';
      }
      tbody.append(tr);// Добавление строк в тэг tbody
  }
  // добавление tbody в таблицу 
  table.appendChild(tbody);

//Функция сортировки таблицы
  let sortTable = function (index,type){
    let tbody = table.querySelector('tbody');// определение тэга таблицы 

    let compare = function(rowA, rowB){// Функция для сравнение элементов столбца
      let rowDataA = rowA.cells[index];
      let rowDataB = rowB.cells[index];

      switch (type){
        //Сортировка таблицы для текстового значения
        case 'text':
          if(rowDataA.innerHTML < rowDataB.innerHTML) return -1;
          else if(rowDataA.innerHTML > rowDataB.innerHTML) return 1;
          return 0  
          break;
          // Сортировка таблицы по цвету
        case 'color':
            if(rowDataA.value < rowDataB.value) return -1;
            else if(rowDataA.value > rowDataB.value) return 1;
            return 0  
          break; 
          //Сортировка таблицы по номеру
        case 'number':
              return rowDataA.innerHTML - rowDataB.innerHTML;
          break; 
      }
    }
    let rows = [].slice.call(tbody.rows);//Определение строк для сортировки
    rows.sort(compare);// СОртировка
  ...
  //Определение заголовков для сортировки
  ...
  function checkEmpty(){
    ...
    if(fname === ''){
      alert('First Name не должно быть пустым!')
      isEmpty = true;
    }
    else if(lname === ''){
      alert('Last Name не должно быть пустым!')
      isEmpty = true;
    }
    else if(about === ''){
      alert('About не должно быть пустым!')
      isEmpty = true;
    }
    else if(eyeColor === ''){
      alert('Eye Color не должно быть пустым!')
      isEmpty = true;
    }
    return isEmpty;
  }
    ...
    // Создание новых строк и ячеек
    //Получение значения из input'a
    //Установка значений в ячейку строки
    let newRow = table.insertRow(table.length),
    //Добавление ячеек в строку по определенному индексу
    cell1 = newRow.insertCell(0),
    cell2 = newRow.insertCell(1),
    cell3 = newRow.insertCell(2),
    cell4 = newRow.insertCell(3),
    cell5 = newRow.insertCell(4),
      //Чтение данных из input'a
    number = document.querySelector('#number').value,
    fname = document.querySelector('#fname').value,
    lname = document.querySelector('#lname').value,
    about = document.querySelector('#about').value,
    ...
    //Заполнение ячеек данными полученными из input'ов
    cell1.innerHTML = number;
    cell2.innerHTML = fname;
    cell3.innerHTML = lname;
    cell4.innerHTML = about;
    
    cell5.value = eyeColor.toLowerCase();
    cell5.style.backgroundColor = eyeColor; 
...
// display selected row data input text
//Функция для получения input'ами значений из ячеек таблицы
function selectRow(){
  for( let i = 2; i < table.rows.length; i++){
    table.rows[i].onclick = function(){
      // get the selecter row index
      rIndex = this.rowIndex;
      document.querySelector('#number').value = this.cells[0].innerHTML;
      document.querySelector('#fname').value = this.cells[1].innerHTML;
      document.querySelector('#lname').value = this.cells[2].innerHTML;
      document.querySelector('#about').value = this.cells[3].innerHTML;
      document.querySelector('#eyeColor').value = this.cells[4].value ;
    };
  }
}
selectRow();
//Функция для изменения Значений выбранной строки
function editRow(){
  ...
  if(!checkEmpty()){
    ...
    table.rows[rIndex].cells[3].innerHTML = about;
    table.rows[rIndex].cells[4].value = eyeColor;
    table.rows[rIndex].cells[4].style.backgroundColor = eyeColor;
  }
}
//Функция удаления строк
function removeRow(){
  table.deleteRow(rIndex);//Удаление строки
  //Очистка input'ов
...
let td = document.querySelectorAll('td');//Определение ячеек таблицы
//Функции для скрытия стобцов таблицы
function hide(){
  for(let i = 0; i < td.length; i = i + 5){//Цикл определения каждой пятой ячейки
    let th = document.querySelector('.th1');//Определение строки
    if(td[i].style.visibility == 'visible'){//Установка значений для скрытия таблицы
      td[i].style.visibility = 'hidden';
      th.innerHTML = 'show';
    }
    else{
      td[i].style.visibility = 'visible';//Установка значений для показа таблицы
      th.innerHTML = 'hide';    
    }
  }
}
function hide1(){
  for(let i = 1; i < td.length; i = i + 5){
    let th = document.querySelector('.th2');
    if(td[i].style.visibility == 'visible'){
      td[i].style.visibility = 'hidden';
      th.innerHTML = 'show';
    }
    else{
...
function hide2(){
  for(let i = 2; i < td.length; i = i + 5){
    let th = document.querySelector('.th3');
    if(td[i].style.visibility == 'visible'){
      td[i].style.visibility = 'hidden';
      th.innerHTML = 'show';
    }
...
function hide3(){
  for(let i = 3; i < td.length; i = i + 5){
    let th = document.querySelector('.th4');
    if(td[i].style.visibility == 'visible'){
      td[i].style.visibility = 'hidden';
      th.innerHTML = 'show';
    }
...
function hide4(){
  for(let i = 4; i < td.length; i = i + 5){
    let th = document.querySelector('.th5');
    if(td[i].style.visibility == 'visible'){
      td[i].style.visibility = 'hidden';
      th.innerHTML = 'show';
    }
...
let rows = document.querySelectorAll('tr');//Определение строк
//Функции для осуществления постраничного вывода
//Функция для вывода первых 10 значений таблицы
...
//Функция для вывода значений таблицы с 11 по 20
function hideTableRows1(){
  for(i = 2; i < rows.length; i++){
    if(i < 12 || i > 21 ){
      rows[i].style.display = 'none';
    }
...
//Функция для вывода значений таблицы с 21 по 30 
function hideTableRows2(){
  for(i = 2; i < rows.length; i++){
    if(i < 22 || i > 31){
      rows[i].style.display = 'none';
    }
...
//Функция для вывода значений таблицы с 31 по 40
function hideTableRows3(){
  for(i = 2; i < rows.length; i++){
    if(i < 32 || i > 41){
      rows[i].style.display = 'none';
    }
...
//Функция для вывода значений таблицы с 41 по 50 
function hideTableRows4(){
  for(i = 2; i < rows.length; i++){
    if(i < 42){
      rows[i].style.display = 'none';
    }
